...
import { ConflatableMessage, MessageLoop } from '@phosphor/messaging';
...
class RankedPanel extends Widget {
     ...
     */
    onChildRemoved(msg) {
        let index = ArrayExt.findFirstIndex(this._items, item => item.widget === msg.child);
        if (index !== -1) {
...
export class NotebookTools extends Widget {
    ...
    _onActiveNotebookPanelMetadataChanged(sender, args) {
        let message = new ObservableJSON.ChangeMessage('activenotebookpanel-metadata-changed', args);
        each(this._toolChildren(), widget => {
    ...
    _onActiveCellMetadataChanged(sender, args) {
        let message = new ObservableJSON.ChangeMessage('activecell-metadata-changed', args);
        each(this._toolChildren(), widget => {
            MessageLoop.sendMessage(widget, message);
        });
    }
...
(function (Private) {
    ...
    function createSelectorNode(options) {
        ...
        let node = VirtualDOM.realize(h.div({}, h.label(title, h.select({}, optionNodes))));
