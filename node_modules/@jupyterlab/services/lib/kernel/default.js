...
class DefaultKernel {
    ...
    constructor(options, id) {
        ...
        this._reconnectAttempt = 0;
        this._isReady = false;
        this._readyPromise = new coreutils_3.PromiseDelegate();
    ...
    async requestKernelInfo() {
        ...
        let reply = (await Private.handleShellMessage(this, msg));
        if (this.isDisposed) {
    ...
    async _handleMessage(msg) {
        ...
        if (msg.parent_header &&
            msg.channel === 'iopub' &&
            (messages_1.KernelMessage.isDisplayDataMsg(msg) ||
                messages_1.KernelMessage.isUpdateDisplayDataMsg(msg) ||
                messages_1.KernelMessage.isExecuteResultMsg(msg))) {
...
(function (Private) {
    ...
    async function startNew(options) {
        ...
        let response = await __1.ServerConnection.makeRequest(url, init, settings);
     ...
     * Restart a kernel.
     */
    async function restartKernel(kernel, settings) {
        ...
        await Promise.all(Private.runningKernels.filter(k => k.id === kernel.id).map(k => k.handleRestart()));
        let response = await __1.ServerConnection.makeRequest(url, init, settings);
        if (response.status !== 200) {
            throw new __1.ServerConnection.ResponseError(response);
        }
     ...
     * Interrupt a kernel.
     */
    async function interruptKernel(kernel, settings) {
        if (kernel.status === 'dead') {
            throw new Error('Kernel is dead');
        }
        settings = settings || __1.ServerConnection.makeSettings();
     ...
     * Delete a kernel.
     */
    async function shutdownKernel(id, settings) {
        settings = settings || __1.ServerConnection.makeSettings();
     ...
     * @returns A promise that resolves when all the kernels are shut down.
     */
    async function shutdownAll(settings) {
        settings = settings || __1.ServerConnection.makeSettings();
     ...
     */
    async function getKernelModel(id, settings) {
        settings = settings || __1.ServerConnection.makeSettings();
        let url = coreutils_1.URLExt.join(settings.baseUrl, KERNEL_SERVICE_URL, encodeURIComponent(id));
        let response = await __1.ServerConnection.makeRequest(url, {}, settings);
        if (response.status !== 200) {
            throw new __1.ServerConnection.ResponseError(response);
        }
        let data = await response.json();
