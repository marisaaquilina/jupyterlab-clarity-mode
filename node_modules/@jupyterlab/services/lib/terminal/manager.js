...
class TerminalManager {
     ...
     * options.
     */
    async connectTo(name, options) {
        const session = await terminal_1.TerminalSession.connectTo(name, this._getOptions(options));
        this._onStarted(session);
        return session;
    }
    /**
    ...
    async shutdown(name) {
        ...
        await terminal_1.TerminalSession.shutdown(name, this.serverSettings);
    }
    /**
    ...
    async shutdownAll() {
        ...
        try {
            ...
            await Promise.all(this._models.map(({ name }) => terminal_1.TerminalSession.shutdown(name, this.serverSettings)));
        }
        finally {
    ...
    async requestRunning() {
        const models = await terminal_1.TerminalSession.listRunning(this.serverSettings).catch(err => {
